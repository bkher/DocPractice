 ...
 * Test class           : TestResults.java
 * Description          : Capture the test results and save it into a txt report
 * 						  and takes backup of already existing results
 */
@Component
public class TestResults {
		...
		try {
			...
			if (file.exists()) {
				...
				if (new File(sResultBackUP).exists()) {
					...
					System.out.println("TestReport.txt is renamed to TestReport-" + getDateTime() + "-R" + iCount
							+ ".txt");
					return true;
				}
			} else {
	...
	public String ExportResultToTxt(String sWriteStr, String sResFile) throws IOException {
		String line;
