...
public class ChildSupportPage extends AbstractTests {
	private Logger logger = LoggerFactory.getLogger(DeclarationCompletePage.class);
	WebDriverWait wait = new WebDriverWait(driver, 240);

	public ChildSupportPage(WebDriver driver) throws Exception {
		try {
			this.driver = driver;
			waitForPageToLoad(driver);
			logger.info("=========== ChildSupportPage is loaded============");
			Thread.sleep(10000);
		} catch (Exception e) {
			throw new Exception("ChildSupportPage could not be loaded within time");
		}
	}
	...
	public void CompleteVerification() throws Exception {
		...
		if (portalParam.ClientName.contains("NAF")) {
			clickButton(driver, getLocator(uiObjParam.ChildSupportPage_UploadArea), "Upload Area");
			DocUpload(portalParam.Platform, portalParam.Pictures, portalParam.Open);
			clickButton(driver, getLocator(uiObjParam.ChildSupportPage_Next), "Upload Area");
			Thread.sleep(10000);
		}
