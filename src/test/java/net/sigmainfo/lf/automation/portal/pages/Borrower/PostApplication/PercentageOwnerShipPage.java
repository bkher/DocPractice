...
public class PercentageOwnerShipPage extends AbstractTests {
	private Logger logger = LoggerFactory.getLogger(DeclarationCompletePage.class);
	WebDriverWait wait = new WebDriverWait(driver, 240);

	public PercentageOwnerShipPage(WebDriver driver) throws Exception {
		try {
			this.driver = driver;
			waitForPageToLoad(driver);
			logger.info("=========== PercentageOwnerShipPage is loaded============");
			Thread.sleep(10000);
		} catch (Exception e) {
			throw new Exception("PercentageOwnerShipPage could not be loaded within time");
		}
	}

	public void CompleteVerification() throws Exception {
		Thread.sleep(10000);
		clickButton(driver, getLocator(uiObjParam.PercentageOwnerShipPage_UploadArea), "Upload Area");
		DocUpload(portalParam.Platform, portalParam.Pictures, portalParam.Open);
		clickButton(driver, getLocator(uiObjParam.PercentageOwnerShipPage_WriteExplanation), "Upload Area");
		enterText(driver, getLocator(uiObjParam.PercentageOwnerShipPage_Editor), "Self");
		clickButton(driver, getLocator(uiObjParam.PercentageOwnerShipPage_Next), "Next");
		Thread.sleep(10000);
	}
