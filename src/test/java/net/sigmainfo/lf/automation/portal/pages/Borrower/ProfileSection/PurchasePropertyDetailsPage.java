...
public class PurchasePropertyDetailsPage extends AbstractTests {
	...
	public PurchasePropertyDetailsPage(WebDriver driver) throws Exception {
		try {
			this.driver = driver;
			waitForPageToLoad(driver);
			assertTrue(
					removeSpecialChar(
							driver.findElement(getLocator(uiObjParam.purchasePropertyDetailsPage_pageLabel)).getText())
									.contains(removeSpecialChar("Tell us about the property you�re buying.")),
					"Page title does not contain " + "Tell us about the property you�re buying");

			logger.info("=========== PurchasePropertyDetailsPage is loaded============");
		} catch (Exception e) {
			throw new Exception("PurchasePropertyDetailsPage could not be loaded within time");
		}
	}
	...
	public void providePropertyDetails(WebDriver driver) throws Exception {
		Thread.sleep(5000);
		selectBoolean(getLocator(uiObjParam.purchasePropertyDetailsPage_selectedProperty), portalParam.isProperty);
		Thread.sleep(5000);
		selectBoolean(getLocator(uiObjParam.purchasePropertyDetailsPage_inContract), portalParam.isContract);
		Thread.sleep(4000);
		selectFromDropdown(getLocator(uiObjParam.purchasePropertyDetailsPage_propertyTypeDropdown),
				portalParam.propertyType);
		if (portalParam.propertyType.equalsIgnoreCase("Multi-Family")) {
			selectFromDropdown(getLocator(uiObjParam.purchasePropertyDetailsPage_numberOfUnitsDropdown),
