...
public class SocialSecurityAwardPage extends AbstractTests {
	private Logger logger = LoggerFactory.getLogger(DeclarationCompletePage.class);
	WebDriverWait wait = new WebDriverWait(driver, 240);

	public SocialSecurityAwardPage(WebDriver driver) throws Exception {
		try {
			this.driver = driver;
			waitForPageToLoad(driver);
			logger.info("=========== SocialSecurityAwardPage is loaded============");
			Thread.sleep(10000);
		} catch (Exception e) {
			throw new Exception("SocialSecurityAwardPage could not be loaded within time");
		}
	}

	public void CompleteVerification() throws Exception {
		Thread.sleep(10000);
		clickButton(driver, getLocator(uiObjParam.SocialSecurityAwardPage_UploadArea), "Upload Area");
		DocUpload(portalParam.Platform, portalParam.Pictures, portalParam.Open);
		clickButton(driver, getLocator(uiObjParam.SocialSecurityAwardPage_WriteExplanation), "Upload Area");
		enterText(driver, getLocator(uiObjParam.SocialSecurityAwardPage_Editor), "Self");
		clickButton(driver, getLocator(uiObjParam.SocialSecurityAwardPage_Next), "Next");
		Thread.sleep(10000);
